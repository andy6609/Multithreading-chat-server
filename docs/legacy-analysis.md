...
---
## 4. Legacy Architecture Overview (Conceptual)
...
└─────────────────────────────────────────────────────────────┘
                          ...
                          │
┌─────────────────────────────────────────────────────────────┐
│              ConnectionHandler 스레드                        │
│  ┌──────────────────────────────────────────────────────┐  │
│  │  • 연결 수락                                          │  │
│  │  • 상태 관리 (중앙 집중식)                            │  │
│  │  • 모든 클라이언트에게 직접 메서드 호출               │  │
│  └──────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────┘
         │
         │ 스레드 생성
...
│   스레드 1   │  │   스레드 2   │  │   스레드 3   │  │   스레드 N   │
│              │  │              │  │              │  │              │
│ • 블로킹 I/O │  │ • 블로킹 I/O │  │ • 블로킹 I/O │  │ • 블로킹 I/O │
│   대기 중    │  │   대기 중    │  │   대기 중    │  │   대기 중    │
│              │  │              │  │              │  │              │
│ ┌──────────┐ │  │ ┌──────────┐ │  │ ┌──────────┐ │  │ ┌──────────┐ │
│ │직접 호출 │ │  │ │직접 호출 │ │  │ │직접 호출 │ │  │ │직접 호출 │ │
│ │→ 상태    │ │  │ │→ 상태    │ │  │ │→ 상태    │ │  │ │→ 상태    │ │
│ │→ CH      │ │  │ │→ CH      │ │  │ │→ CH      │ │  │ │→ CH      │ │
│ └──────────┘ │  │ └──────────┘ │  │ └──────────┘ │  │ └──────────┘ │
└──────┬───────┘  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘
...
1. 중앙집중식 상태 관리
   ┌─────────────┐
   │  공유 상태  │ ← 모든 스레드가 동시 접근 (경합 발생)
   └─────────────┘

2. 직접 메서드 호출 (스레드 경계 넘나듦)
   ClientHandler ──직접 호출──> ConnectionHandler
   ConnectionHandler ──직접 호출──> ClientHandler
   
3. 블로킹 I/O (각 클라이언트마다 OS 스레드)
   클라이언트 → [블로킹 대기] → ClientHandler 스레드
...
This architecture centralizes connection management and state mutation
around shared mutable state protected by locks,
which introduces scalability and maintainability limitations
under high concurrency.
These issues are structural and cannot be resolved through minor refactoring.
---
